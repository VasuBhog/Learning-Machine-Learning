...
%figure(1), %imshow(img),title('Leaf');
img=double(img)/255;
...
%%figure
%imshow(BW3),title('Black and White Image');

%% erosion-dilation
se = strel('disk',4,4);
imgBWd=imerode(imgBW,se);
%%figure,
%imshow(imgBWd),title('Eroded Image');
se = strel('disk',8,8);
imgBWe=imdilate(imgBWd,se);
%%figure,
%imshow(imgBWe),title('Dilated Image');
se = strel('disk',4,4);
...
%%figure,
%imshow(imgCropped),title('Black and White Image');
